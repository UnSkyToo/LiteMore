namespace LiteFramework.Core.Security
{
    public static class Crc32
    {
        public static uint Calculate(string Name)
        {
            var Buffer = System.Text.Encoding.Default.GetBytes(Name);
        ...
        public static uint Calculate(byte[] Buffer)
        {
            ...
            if (Buffer[Index] != 0)
            {
                Result |= (uint)(Buffer[Index++]) << 16;
                if (Buffer[Index] != 0)
                {
                    Result |= (uint)(Buffer[Index++]) << 8;
                    if (Buffer[Index] != 0)
                    {
